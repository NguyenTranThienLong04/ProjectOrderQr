import { OrderNotePreview } from './OrderNotePreview';
...
export function CustomerMenu() {
  ...
  const menuSearch = useMenuSearch(tableId, lang);
  const search = menuSearch.query;
  const setSearch = menuSearch.reset;
  const [selectedCategory, setSelectedCategory] = useState('all');
  const [selectedDish, setSelectedDish] = useState<PublicDish | null>(null);
  const [dishNote, setDishNote] = useState('');
  const [acceptedAnalysis, setAcceptedAnalysis] = useState<{ key: string; token: string } | null>(null);
  const analysisKey = JSON.stringify([sharedCart?.sessionId, tableId, selectedDish?._id, dishNote]);
  ...
  const semanticDishes = menuSearch.response && !menuSearch.response.fallbackUsed ? menuSearch.response.result.dishes : null;
  const { keywords: _recommendationKeywords, ...recommendationConstraints } = menuSearch.response?.result.appliedFilters ?? {};
  const visibleDishes = (semanticDishes ?? dishes).filter((dish) => (selectedCategory === 'all' || dish.categoryId === selectedCategory) && (semanticDishes !== null || dish.name.toLowerCase().includes(search.trim().toLowerCase())));
  ...
  const openDish = (dish: PublicDish) => { setSelectedDish(dish); setDishNote(''); setAcceptedAnalysis(null); };
  const closeDish = () => { setSelectedDish(null); setDishNote(''); setAcceptedAnalysis(null); };
  const addDish = (dish: PublicDish, note?: string, analysisToken?: string) => emitCart('cart:add', { dishId: dish._id, quantity: 1, note, ...(analysisToken ? { analysisToken, confirmedByCustomer: true } : {}) });
  ...
  return <main id="main-content" tabIndex={-1} className={`min-h-dvh bg-[#f8f5ef] text-stone-900 ${cartCount > 0 ? 'pb-24 sm:pb-8' : 'pb-8'}`}>
      ...
      {menu && <>
        ...
        {paymentSummary && paymentSummary.orders.length > 0 && <section aria-labelledby="session-payment-title" className="mb-6 overflow-hidden rounded-xl border border-stone-200 bg-white">
          ...
          <div className="grid gap-5 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.72fr)]">
            <OrderProgress orders={paymentSummary.orders} lang={lang} />
        ...
        <section aria-labelledby="menu-title"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-accent-600">Thực đơn tại bàn</p><h2 id="menu-title" className="mt-1 text-2xl font-bold tracking-tight text-stone-950">Chọn món bạn thích</h2></div>
          <MenuSearch search={menuSearch} lang={lang} onSubmit={() => { setSelectedCategory('all'); void menuSearch.submit(); }} />
          {sharedCart && <MenuRecommendations cart={sharedCart} tableId={tableId} lang={lang} constraints={recommendationConstraints} paused={!!search.trim() && (menuSearch.loading || !menuSearch.response || menuSearch.response.fallbackUsed)} onSelect={openDish} />}
    ...
    <Modal open={Boolean(selectedDish)} onClose={closeDish} title={selectedDish?.name ?? 'Chi tiết món'} size="md" footer={<div className="space-y-3"><div className="flex items-center justify-between gap-3"><span className="text-sm font-medium text-stone-600">Giá món</span><strong className="tabular-nums text-lg font-bold text-accent-600">{selectedDish ? currency.format(selectedDish.price) : ''}</strong></div><button onClick={() => { if (selectedDish) addDish(selectedDish, dishNote.trim() || undefined, acceptedAnalysis?.key === analysisKey ? acceptedAnalysis.token : undefined); closeDish(); }} disabled={isSyncing || !selectedDish?.isAvailable} className={`${primaryButtonClass} w-full bg-accent-600 hover:bg-accent-500`}><Plus aria-hidden="true" className="h-4 w-4" />Thêm vào giỏ</button></div>}>
      {selectedDish && <div><div className="relative aspect-[4/3] w-full rounded-lg bg-stone-100">{selectedDish.imageUrl ? <img src={selectedDish.imageUrl} alt={selectedDish.name} className="absolute inset-0 m-auto block h-auto w-auto max-h-full max-w-full object-contain" /> : <span className="grid h-full place-items-center"><ImageOff aria-hidden="true" className="h-8 w-8 text-stone-400" /></span>}</div><p className="mt-4 text-sm leading-6 text-stone-600">{selectedDish.description || 'Món ăn hiện chưa có mô tả chi tiết.'}</p><label htmlFor="dish-note" className="mt-5 flex items-center gap-2 text-sm font-semibold text-stone-800"><StickyNote aria-hidden="true" className="h-4 w-4 text-amber-700" />Bạn có nhắn gì với nhà hàng không?</label><textarea id="dish-note" rows={3} maxLength={250} value={dishNote} onChange={(event) => { setDishNote(event.target.value); setAcceptedAnalysis(null); }} aria-describedby="dish-note-help dish-note-count" placeholder="Ví dụ: Ít cay, không hành, cho thêm nước chấm" className={`${controlClass} mt-2 resize-y`} /><div className="mt-1.5 flex items-start justify-between gap-3 text-xs leading-5 text-stone-500"><p id="dish-note-help">Không bắt buộc. Ghi chú này chỉ áp dụng cho món đang chọn.</p><span id="dish-note-count" className="shrink-0 tabular-nums">{dishNote.length}/250</span></div><OrderNotePreview key={analysisKey} sessionId={sharedCart?.sessionId ?? ''} tableId={tableId} dishId={selectedDish._id} note={dishNote} acceptedToken={acceptedAnalysis?.key === analysisKey ? acceptedAnalysis.token : undefined} onAccept={(token) => setAcceptedAnalysis(token ? { key: analysisKey, token } : null)} /></div>}
    </Modal>
